...
## Architecture, Operational Transformation, and Scaling — Comprehensive Documentation
...
<a id="part-12"></a>
# Part 12: Bugs Found During Development

## 12.1 The `transformDeleteInsert` Position Formula (Phase 1)

**Symptom:** the property test found a real divergence with 3+ concurrent operations. **Root cause:** the split-piece position formula assumed sequential left-then-right application (`delStart + insertLen`), but the pieces actually share one coordinate frame. **Fix:** changed to `insertPos + insertLen`, applied via descending-position order (Part 4.3/4.5). Caught exactly as a property test is supposed to catch this kind of thing.

## 12.2 Client-Side TP2 Divergence (Phase 3, CP 3.6)

**Symptom:** a Node.js two-client simulation found a ~1-in-3-4-run divergence. **Root cause:** when a client has 2+ of its own pending operations in flight (guaranteed every time by select-and-replace, which diffs to delete+insert sent back to back), chain-transforming an incoming concurrent op through multiple pending pieces could land it at an *artificial* position collision — the same TP1-vs-TP2 limitation from Part 4.4, re-emerging client-side, where the client has no sequencer to fall back on. **Fix:** serialize client sends (one outstanding op at a time) plus self-healing resync whenever the pending queue empties.

## 12.3 `baseRevision` Staleness Under Fast Typing (Phase 4, CP 4.4)

**Symptom:** typing faster than one network round-trip silently lost all but the first few characters. **Root cause:** `editor.js` stamped each operation's `baseRevision` at *queue* time, not *send* time; by the time a later keystroke actually transmitted, the server's known revision had already advanced past what that keystroke's `baseRevision` claimed — the server ran it through the gap-transform a second time, double-shifting it out of bounds, throwing inside an **unhandled async callback** that silently swallowed the exception (Part 6.1). **Fix:** stamp `baseRevision` at actual transmission time; harden every async chain with `.exceptionally()` so a future bug like this fails loudly instead of silently.

## 12.4 The Cold-Start Cache Bug (Phase 5, CP 5.3)

**Symptom:** none yet — found by *tracing through* the cold-start code path, not by a failure. **Root cause:** `DocumentCommitter.commit()` defaulted to an empty string for any document not yet touched in the current process's lifetime, regardless of how much real history Postgres held for it — meaning the first edit to a pre-existing document right after a restart would silently corrupt it. **Fix:** lazy rebuild-from-log (Part 8.3), verified by deliberately reverting the fix and confirming the predicted failure actually occurred before re-applying it.
...
<a id="part-15"></a>
# Part 15: Quick-Recap Q&A (Interview Prep Style)

**Q: What's the difference between OT and CRDTs, and why does it matter for system design?**
A: OT transforms operations against each other relative to *positions*, which requires resolving the gap between when an op was created and what's committed since — and that resolution either needs a central sequencer (this project's choice) or runs into the TP2 problem for 3+ concurrent edits. CRDTs sidestep that entirely by giving every element a stable, never-changing identity, so concurrent operations are commutative by construction — at the cost of per-element identifier overhead and (usually) tombstones for deletes.

**Q: Why doesn't this project need to solve TP2?**
A: Because it has a single authoritative sequencer per document — only one arrival order for concurrent operations ever actually occurs in practice, so N-way arrival-order independence is a guarantee the architecture never needs. This is also how Google Docs' real production OT system works.

**Q: How do you guarantee an operation that fails to persist never gets acknowledged?**
A: The ack is only ever sent inside a `.thenAccept()` chained after the durable write's `CompletableFuture` — if the write throws, that future completes exceptionally, `.thenAccept()` never runs, and `.exceptionally()` logs the failure instead. No ack, no broadcast, by construction of the chain, not by an explicit if-check.

**Q: Without locks, how do you prevent two concurrent edits to the same document from corrupting it?**
A: A dedicated single-threaded executor per document. Only one thread is ever allowed to touch a given document's state, enforced by routing every operation through that document's executor — the absence of concurrent access is what makes a lock unnecessary, not a lock itself.

**Q: What broke when you split `connection-tier` and `document-service` into separate processes?**
A: The atomic "register as broadcast peer + read history" guarantee, which used to live for free inside `document-service`'s own per-document executor. Once `document-service` had no idea `connection-tier`'s peer list existed, `connection-tier` had to rebuild that atomicity itself, with its own per-document executor (`DocumentTaskSequencer`).

**Q: How would you scale this to millions of concurrent documents?**
A: Shard `document-service` by consistent hashing on `documentId`; introduce a pub/sub bus so multiple `connection-tier` instances can broadcast across each other (today's design assumes every editor of a document is connected to the *same* `connection-tier` process, which breaks the moment there's more than one); partition Postgres's op log by document and add read replicas; `document-metadata-service` and `frontend-server` already scale horizontally for free since they're stateless.

**Q: Why is the document's content never stored directly anywhere, only the op log?**
A: A deliberate decision for this learning project — no snapshotting, ever. The current text is always the replay of every committed operation from revision 1. This makes the system's actual mechanics fully inspectable (you can literally read every keystroke back out of Postgres) at the cost of unbounded storage growth, which would need a real compaction strategy in production.

**Q: What's the actual mechanism that makes `ack-after-durable-write` correct, structurally, not just by convention?**
A: `DocumentCommitter.commit()` calls the durable write synchronously, inside the same call that produces the result the ack is built from — there's no path to constructing an ack without that write having already either succeeded (committed) or thrown (caught by `.exceptionally()`, no ack constructed at all).

**Q: Why gRPC over REST for service-to-service calls here?**
A: Binary protobuf + HTTP/2 multiplexing beats JSON + HTTP/1.1 for a tight request/response loop, and the `.proto` schema enforces a strict contract both sides compile against — though the honest caveat is that the real bottleneck in this pipeline is the synchronous Postgres write, not the wire protocol, so the speed difference, while real, isn't what was actually limiting throughput.
